...
void GhiFileDSMB(const char* fileName, LISTMB LMB)
{
    ...
    if (file.is_open())
    {
        ...
        for (int i = 0; i < LMB.n; i++)
        {
            file << LMB.nodes[i]->SoHieuMB << "\n";
            file << LMB.nodes[i]->LoaiMB << "\n";
            file << LMB.nodes[i]->soDay << "\n";
            file << LMB.nodes[i]->soDong << "\n\n";
        }
        file.close();
    }
    else
...
void DocFileDSMB(const char* fileName, LISTMB& LMB)
{
    MayBay MB;
    ...
    if (err == 0)
    {
        ...
        if (!feof(f))
        {
            ...
            for (int i = 0; i < LMB.n; i++)
            {
                fscanf_s(f, "%s", MB.SoHieuMB, static_cast<unsigned int>(sizeof(MB.SoHieuMB)));
                fgets(MB.LoaiMB, sizeof(MB.LoaiMB), f);
                fgets(MB.LoaiMB, sizeof(MB.LoaiMB), f);
                LoaiBoKiTuXuongDong(MB.LoaiMB);
                fscanf_s(f, "%d", &MB.soDay);
                fscanf_s(f, "%d", &MB.soDong);
...
void NhapDSMB(LISTMB& LMB)
{
    ...
    do
    {
        ...
        SHMB:
            ...
            gotoxy(57, current);
            fflush(stdin);
            NhapChuoi1(MB.LoaiMB, sizeof(MB.LoaiMB), 57, current);
            DelSpace(MB.LoaiMB);
...
{
    NODECB p, t = nullptr, s; //p nut trc,q nut sau
...
{
    NODECB p, t = NULL, s; // Initialize t to NULL
    p = new ListCB;
    p->CB = CB;
...
void GhiFileDSCB(const char* fileName, NODECB LCB)
{
    ...
    if (file.is_open())
    {
        ...
        {
            file << p->CB.MaCB << "\n"
                << p->CB.NgayKhoiHanh.hour << " " << p->CB.NgayKhoiHanh.min << " " << p->CB.NgayKhoiHanh.day << " " << p->CB.NgayKhoiHanh.month << " " << p->CB.NgayKhoiHanh.year << "\n"
                << p->CB.SanBayDen << "\n"
                << p->CB.TrangThai << "\n"
                << p->CB.SoHieuMB << "\n"
                << p->CB.SoVeDaDat << "\n"
            ...
            if (p->CB.SoVeDaDat > 0)
            {
                ...
                {
                    file << p->CB.dsVe[i].ViTriNgoi << " " << p->CB.dsVe[i].CMND << " ";
                }
            }
            file << "\n\n";
        }
...
void DocFileDSCB(const char* fileName, NODECB& LCB)
{
    ChuyenBay CB;
    ifstream file(fileName); // Mở file để đọc

    if (file.is_open()) // Kiểm tra xem file đã mở thành công hay chưa
    {
        // Đọc dữ liệu từ file
        while (file >> CB.MaCB >> CB.NgayKhoiHanh.hour >> CB.NgayKhoiHanh.min >> CB.NgayKhoiHanh.day >> CB.NgayKhoiHanh.month >> CB.NgayKhoiHanh.year)
        {
            file.ignore(); // Bỏ qua ký tự xuống dòng
            file.getline(CB.SanBayDen, sizeof(CB.SanBayDen)); // Đọc dòng chứa tên sân bay đến

            LoaiBoKiTuXuongDong(CB.SanBayDen); // Loại bỏ ký tự xuống dòng cuối chuỗi

            file >> CB.TrangThai >>CB.SoHieuMB>> CB.SoVeDaDat >> CB.MaxVe;

            CB.dsVe = new Ve[CB.MaxVe];

            if (CB.SoVeDaDat > 0)
            {
                for (int i = 0; i < CB.MaxVe; i++)
                {
                    file >> CB.dsVe[i].ViTriNgoi >> CB.dsVe[i].CMND;
                }
            }
            if (Time(CB.NgayKhoiHanh) <= 0 && CB.SoVeDaDat == 0)
                CB.TrangThai = 0;
...
void NhapCB(NODECB& LCB, LISTMB LMB)
{
    ...
    SANBAYDEN:
        ...
        cout << "               ";
        gotoxy(76, current);
        NhapChuoi1(CB.SanBayDen,50, 76, current);
    ...
    SHMB:
        ...
        if (a != 0)
        {
            ...
            {
                if (abs(Time(h->CB.NgayKhoiHanh) - t) < setupTimeDatSHMB)
                {
    ...
    // Giải phóng bộ nhớ CB.dsVe
    for (NODECB p = LCB; p != NULL; p = p->next)
    {
        delete[] p->CB.dsVe;
    }
}
...
SHMB:
    ...
    if (a != 0)
    {
        ...
        for (h = tmpCheckCB; h != NULL;)
        {
            if (h->CB.MaCB != NULL && abs(Time(h->CB.NgayKhoiHanh) - t) < setupTimeDatSHMB) // trg hợp thoi gian dat nho hon time trong ds time tang dan
            {
                AlertErr(8, 4, "HAI CHUYEN BAY CUNG SO HIEU PHAI CACH IT NHAT 5 NGAY ");
                Sleep(3000);
                clearErr(8, 4);
...
void GhiFileDSKH(char* fileName, BSTKH LKH)
{
    FILE* f;
...
void DocFileDSKH(const char* fileName, BSTKH& LKH)
{
    ThongTinKH KH;
    ...
    {
        fseek(f, 0, SEEK_END);
        unsigned int fileSize = static_cast<unsigned int>(ftell(f));
        if (fileSize == 0)
        {
            fclose(f);
            return;
        ...
        while (!feof(f))
        {
            fscanf_s(f, "%s", KH.CMND, static_cast<unsigned int>(_countof(KH.CMND)));
            fgets(KH.Ho, sizeof(KH.Ho), f);
            fgets(KH.Ten, sizeof(KH.Ten), f);
            fgets(KH.Phai, sizeof(KH.Phai), f);

            // Remove newline characters from Ho, Ten, and Phai
            LoaiBoKiTuXuongDong(KH.Ho);
            LoaiBoKiTuXuongDong(KH.Ten);
            LoaiBoKiTuXuongDong(KH.Phai);
...
BSTKH SearchKH(BSTKH LKH, char CMND[13])
{
    BSTKH p;
    p = LKH;
...
void NhapKH(BSTKH& LKH, char CMND[13], int soDay)
{
    ThongTinKH KH;
    strcpy_s(KH.CMND, CMND);
    int y = 9 + soDay * 2;
    gotoxy(37, y + 10);
    SetColor(Yellow);
    ...
    fflush(stdin);
    SetColor(White);
    NhapChuoi1(KH.Ho, sizeof(KH.Ho), 34, y + 14);
    StandardName(KH.Ho);
    LoaiBoKiTuXuongDong(KH.Ho);
    SetColor(Yellow);
    gotoxy(60, y + 8);
...
void Inorder(BSTKH KH)
{
    ...
    if (KH != NULL)
    {
        gotoxy(100, y);
        Inorder(KH->left);
        printf("%d ",atoi(KH->data.CMND)); 
        y++;
        Inorder(KH->right);
    }
    else
    {
        return; // Kết thúc hàm khi KH là NULL
    }
}
...
void NhapVe(NODECB& LCB, NODECB& p, NODECB tmp, int x, int y, int soDong, int soDAY, BSTKH LKH)
{
    for (int i = 0; i < 13; i++)
        clearErr(8, 10 + soDAY * 2 + 4 + i);

    char vitringoi[5];
    sprintf_s(vitringoi, "%c%d", char(y + 64), x);
    gotoxy(8, 4);
...
CMND:
    ...
    cout << "NHAP VAO CMND: ";
    if (inputNum(p->CB.dsVe[p->CB.SoVeDaDat].CMND, 12) == -1)
    {
        for (int i = 0; i < 13; i++)
    ...
    for (NODECB q = tmp; q != NULL; q = q->next)
    {
    ...
    if (strcmp(confirm, "yes") == 0)
    {
        for (int y = 7; y < 20; y++)
        {
            ...
            clearErr(8, 8 + soDAY * 2 + 4 + y - 7);
        }

        strcpy_s(p->CB.dsVe[p->CB.SoVeDaDat].ViTriNgoi, vitringoi);
        p->CB.SoVeDaDat++;
...
void DSKHOfCB(NODECB LCB, BSTKH LKH, int current)
{
    ...
    for (int i = 0; i < p->CB.SoVeDaDat; i++)
    {
        ...
        strcpy_s(HoTen, KH->data.Ho);
        strcat_s(HoTen, " ");
        strcat_s(HoTen, KH->data.Ten);
