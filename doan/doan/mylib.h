...
void TextColor(int color)//mau chu
{
...
void gotoXY(int column, int line)//di chuyen toi toa do x,y
{
    COORD coord;
    coord.X = column;
    coord.Y = line;
...
void PutCharXY(int x, int y, const char* KyTu, int color)//Dat tai vi tri xy va in ra voi mau sac tuy chon
{
    gotoXY(x, y);
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), color);
    printf(KyTu);
...
void AlertErr(int x, int y, const char* s)
{
    gotoxy(x, y);
    SetColor(12);
    SetBGColor(0);
...
void NhapChuoi1(char* S, int size, int x, int y) {
    fflush(stdin);
    do {
        gotoxy(x, y);
        fgets(S, size, stdin);
